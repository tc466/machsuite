...
#define domainEdge ((TYPE)6.0)
// LJ1=4e(s^12), LJ2=4e(s^6) -> s=1.049
#define van_der_Waals_thresh ((TYPE)(0.5*0.5))
...
void generate_binary()
{
  ...
  TYPE x, y, z;
  const TYPE infinity = (domainEdge*domainEdge* ((TYPE)3.));//(max length)^2 * 1000
  ...
  while( i<nAtoms ) {
    ...
    x = domainEdge*((TYPE) (((double)random())/((double)RAND_MAX)));
    y = domainEdge*((TYPE) (((double)random())/((double)RAND_MAX)));
    z = domainEdge*((TYPE) (((double)random())/((double)RAND_MAX)));
  ...
  memset(data.d_force_x, 0, nAtoms*sizeof(TYPE));
  memset(data.d_force_y, 0, nAtoms*sizeof(TYPE));
  memset(data.d_force_z, 0, nAtoms*sizeof(TYPE));
